...
func main() {
	bot, err := tgbotapi.NewBotAPI("5423934655:AAE-zUdG3KAP1L7bvHC_Q-U5CkS4CtLdGgA")
	if err != nil {
		log.Panic(err)
	}
	...
	for update := range updates {
		if update.Message != nil { // If we got a message
			...
			command := strings.Split(update.Message.Text, " ") //разделяем вводимую строку на пробелы для чтения слов
